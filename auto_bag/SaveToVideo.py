...
def acquire_images(cam, nodemap):
    ...
    try:
        ...
        if PySpin.IsAvailable(node_framerate) and PySpin.IsWritable(node_framerate):
            node_framerate.SetValue(20.1)  # or 60.0 if your camera supports it
            print("Capture FPS set to", node_framerate.GetValue())
	'''
...
def run_capture_and_save(destination_folder):
    # The main logic of the example should go here, using destination_folder to save the video
    if not os.path.exists(destination_folder):
        os.makedirs(destination_folder)
    os.chdir(destination_folder)
    main()  # Assuming main() already captures and saves the video
